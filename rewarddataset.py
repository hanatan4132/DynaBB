...
def collect_data_with_auto_strategy(env, total_steps):
    ...
    while collected_count < total_steps:
        ...
        _, reward1, done, _ = env.step(action)
        current_frame2 = get_frame(env)
        _, reward2, done, _ = env.step(action)
        attempted_steps += 1
        if reward1 != 0 :
            reward = reward1
        else:
        ...
        if keep_sample:
            try:
                stacked_frames = np.stack([current_frame,current_frame2], axis=0)
...
# 收集資料 (保持總資料量 5000 筆)
print("開始自動收集獎勵資料...")
attempted_steps = collect_data_with_auto_strategy(env, 20000)
